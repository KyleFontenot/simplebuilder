...
import {readFileSync, readdirSync, lstatSync, copyFileSync, writeFileSync, mkdirSync} from "node:fs"
...
async function walkStructure(template: string){
  const startpoint = "./src/routes/"
  
  function beginWalk(start: string){
    const absLocation = resolve(start);
    console.log("aboslute llocation",absLocation)
          ...
          switch (extension) {
              ...
              case '.md':
                ...
                if(file.startsWith("+")){
                    ...
                    console.log("md parsed", mdParsed)
